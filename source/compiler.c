...
/*!
\brief code block prelude
\param out output file descriptor
\param p position in tree
\param fn function name
**/

static void code_block_prelude (NODE_T * p, FILE_T out)
{
  (void) p;
  indentf (out, snprintf (line, BUFFER_SIZE, "{\n"));
  indentation ++;
  temp_book_pointer = 0;
}

/*!
\brief code block postlude
\param out output file descriptor
\param p position in tree
\param fn function name
**/

static void code_block_postlude (NODE_T * p, FILE_T out)
{
  (void) p;
  indentation --;
  indentf (out, snprintf (line, BUFFER_SIZE, "}\n"));
...
static void code_dereference_identifier (NODE_T * p, FILE_T out, int phase)
{
  ...
  if (phase == L_DECLARE) {
    if (temp_booked (BOOK_DEREF, L_DECLARE, SYMBOL (q)) != NOT_BOOKED) {
      ...
      add_declaration (&root_idf, code_mode (MOID (p)), 1, idf); 
...
static void code_dereference_slice (NODE_T * p, FILE_T out, int phase)
{
  ...
  if (phase == L_DECLARE) {
    ...
    if (entry == NOT_BOOKED) {
      ...
      add_declaration (&root_idf, code_mode (mode), 1, drf);
...
static void code_dereference_selection (NODE_T * p, FILE_T out, int phase)
{
  ...
  if (phase == L_DECLARE) {
    ...
    if (entry == NOT_BOOKED || field_idf != (char *) (entry->info)) { 
      ...
      add_declaration (&root_idf, code_mode (SUB_MOID (field)), 1, sel);
...
static void code_selection (NODE_T * p, FILE_T out, int phase)
{
  ...
  if (phase == L_DECLARE) {
    ...
    if (entry == NOT_BOOKED || field_idf != (char *) (entry->info)) { 
      ...
      add_declaration (&root_idf, code_mode (MOID (field)), 1, sel);
      book_temp (BOOK_DECL, L_DECLARE, SYMBOL (idf), (void *) field_idf, NUMBER (field)); 
    }
    code_unit (sec, out, L_DECLARE);
  } else if (phase == L_EXECUTE) {
...
static char * compile_denotation (NODE_T * p, FILE_T out, int fun_or_block)
{
/*
...
*/
  ...
  if (primitive_mode (MOID (p)) && fun_or_block == A68_MAKE_FUNCTION) {
...
static char * compile_cast (NODE_T * p, FILE_T out, int fun_or_block)
{
  if (basic_unit (p)) {
    static char fn[NAME_SIZE];
    (void) make_name (fn, "_cast", "", NUMBER (p));
    comment_source (p, out);
    if (fun_or_block == A68_MAKE_FUNCTION) {
      code_fun_prelude (p, out, fn);
    }
    root_idf = NULL;
...
static char * compile_identifier (NODE_T * p, FILE_T out, int fun_or_block)
{
...
static char * compile_dereference_identifier (NODE_T * p, FILE_T out, int fun_or_block)
{
  if (basic_mode (MOID (p)) && basic_unit (p)) {
    static char fn[NAME_SIZE];
...
static char * compile_slice (NODE_T * p, FILE_T out, int fun_or_block)
{
  if (basic_mode (MOID (p)) && basic_unit (p)) {
    static char fn[NAME_SIZE];
...
static char * compile_dereference_slice (NODE_T * p, FILE_T out, int fun_or_block)
{
  if (basic_mode (MOID (p)) && basic_unit (p)) {
    static char fn[NAME_SIZE];
...
static char * compile_selection (NODE_T * p, FILE_T out, int fun_or_block)
{
  if (basic_mode (MOID (p)) && basic_unit (p)) {
    static char fn[NAME_SIZE];
...
static char * compile_dereference_selection (NODE_T * p, FILE_T out, int fun_or_block)
{
  if (basic_mode (MOID (p)) && basic_unit (p)) {
    static char fn[NAME_SIZE];
...
static char * compile_formula (NODE_T * p, FILE_T out, int fun_or_block)
{
  if (basic_unit (p)) {
    static char fn[NAME_SIZE];
...
static char * compile_voiding_formula (NODE_T * p, FILE_T out, int fun_or_block)
{
  if (basic_unit (p)) {
    static char fn[NAME_SIZE];
...
static void code_arguments (NODE_T * p, FILE_T out, int phase, int * size)
{
  if (p == NULL) {
    ...
    if (phase == L_DECLARE) {
      add_declaration (&root_idf, code_mode (MOID (p)), 1, arg); 
      code_unit (p, out, L_DECLARE);
...
static char * compile_deproceduring (NODE_T * p, FILE_T out, int fun_or_block)
{
...
/* Declare */
    ...
    print_declarations (out, root_idf);
/* Initialise */
    if (fun_or_block != A68_MAKE_NOTHING) {
...
static char * compile_voiding_deproceduring (NODE_T * p, FILE_T out, int fun_or_block)
{
...
static char * compile_call (NODE_T * p, FILE_T out, int fun_or_block)
{
  NODE_T * proc = SUB (p);
...
static char * compile_voiding_call (NODE_T * p, FILE_T out, int fun_or_block)
{
...
char * compile_voiding_assignation_selection (NODE_T * p, FILE_T out, int fun_or_block)
{
...
static char * compile_voiding_assignation_slice (NODE_T * p, FILE_T out, int fun_or_block)
{
  NODE_T * dst = SUB (locate (p, ASSIGNATION));
  NODE_T * src = NEXT_NEXT (dst);
...
static char * compile_voiding_assignation_identifier (NODE_T * p, FILE_T out, int fun_or_block)
{
  NODE_T * dst = SUB (locate (p, ASSIGNATION));
  NODE_T * src = NEXT_NEXT (dst);
...
/* Declare */
    ...
    if (temp_booked (BOOK_DEREF, L_DECLARE, SYMBOL (q)) == NOT_BOOKED) {
      (void) make_name (idf, SYMBOL (q), "", NUMBER (p));
      add_declaration (&root_idf, code_mode (SUB_MOID (dst)), 1, idf);
...
static char * compile_identity_relation (NODE_T * p, FILE_T out, int fun_or_block)
{
#define GOOD(p) (locate (p, IDENTIFIER) != NULL && WHETHER (MOID (locate ((p), IDENTIFIER)), REF_SYMBOL))
  NODE_T * lhs = SUB (p);
  NODE_T * op = NEXT (lhs);
...
static void compile_serial_clause (NODE_T * p, FILE_T out, int pop, int fun_or_block)
{
  for (; p != NULL; FORWARD (p)) {
    if (fun_or_block == A68_MAKE_OTHERS) {
      if (WHETHER (p, UNIT) || WHETHER (p, DECLARATION_LIST)) {
        ...
        compile_serial_clause (SUB (p), out, pop, fun_or_block);
      }
...
static char * compile_closed_clause (NODE_T * p, FILE_T out, int fun_or_block)
{
  NODE_T *sc = NEXT_SUB (p);
  if (SYMBOL_TABLE (sc)->labels == NULL) {
    static char fn[NAME_SIZE];
    compile_serial_clause (NEXT_SUB (p), out, NUMBER (p), A68_MAKE_OTHERS);
    ...
    compile_serial_clause (NEXT_SUB (p), out, NUMBER (p), A68_MAKE_FUNCTION);
    indent (out, "CLOSE_FRAME;\n");
    if (NEEDS_DNS (MOID (p))) {
...
static char * compile_collateral_clause (NODE_T * p, FILE_T out, int fun_or_block)
{
...
static char * compile_conditional_clause (NODE_T * p, FILE_T out, int fun_or_block)
{
  static char fn[NAME_SIZE];
  NODE_T * q = SUB (p);
...
{
/**/
#define COMPILE(p, out, fun, fun_or_block) {\
  char * fn = (fun) (p, out, fun_or_block);\
  if (fun_or_block == A68_MAKE_FUNCTION && fn != NULL) {\
...
/**/
  if (p == NULL) {
    ...
    COMPILE (p, out, compile_closed_clause, make_function);
  } else if (WHETHER (p, ENCLOSED_CLAUSE)) {
    COMPILE (SUB (p), out, compile_unit, make_function);
  } else if (WHETHER (p, COLLATERAL_CLAUSE)) {
